...
def generate(num_of_sample, out_dir, start_id=0, run_device="cpu", checkpoint = "checkpoints/g_stat.pt"):
    ...
    netG = Generator()
    checkpoint= torch.load(os.path.join(root_dir, checkpoint), map_location=device)
    ...
    for i in tqdm(range(start_id, start_id  + num_of_sample)):
        noise = torch.Tensor(1, 1, 5000).uniform_(-1, 1)
